...
/*
 * MotorConfigurator, used to configure the SparkFlex to prevent code redundency
 * Enjoy, by Brayden
 */
public class FlexConfigurator {

    private SparkFlexConfig sparkFlexConfig;

    /**
     * Constructs a MotorConfigurator using the provided sparkFlexConfig.
     *
     * @param sparkFlexConfig the SparkFlex configuration object to be configured
     */
    public FlexConfigurator(SparkFlexConfig sparkFlexConfig) {
     ...
     */
    public void applyBasicConfigurations(IdleMode idleMode, boolean inverted, boolean isOpenRamp, double rampRate, int smartCurrentLimit) {
        sparkFlexConfig.idleMode(idleMode);
        sparkFlexConfig.inverted(inverted);
        if (isOpenRamp) {
            sparkFlexConfig.openLoopRampRate(rampRate);
        } else {
            sparkFlexConfig.closedLoopRampRate(rampRate);
        }
        sparkFlexConfig.smartCurrentLimit(smartCurrentLimit);
        /*
         * All signals should remain on to ensure accessibility to information when required.
         */
        sparkFlexConfig.signals.absoluteEncoderPositionAlwaysOn(true);
        sparkFlexConfig.signals.absoluteEncoderVelocityAlwaysOn(true);
        sparkFlexConfig.signals.faultsAlwaysOn(true);
        sparkFlexConfig.signals.iAccumulationAlwaysOn(true);
        sparkFlexConfig.signals.primaryEncoderPositionAlwaysOn(true);
        sparkFlexConfig.signals.primaryEncoderVelocityAlwaysOn(true);
        sparkFlexConfig.signals.warningsAlwaysOn(true);
    }
     ...
     * @param rampRate          the ramp rate value to apply
     * @param smartCurrentLimit the smart current limit to apply
     * @param canLeader         the SparkFlex to follow
     */
    public void applyConfigurations(IdleMode idleMode, boolean inverted, boolean isOpenRamp, double rampRate, int smartCurrentLimit, SparkFlex canLeader) {
        applyBasicConfigurations(idleMode, inverted, isOpenRamp, rampRate, smartCurrentLimit);
        sparkFlexConfig.follow(canLeader, inverted);
    }
    ...
    public void applyConfigurations(IdleMode idleMode, boolean inverted, boolean isOpenRamp, double rampRate, int smartCurrentLimit, boolean isAbsolute, double positionConversionFactor) {
        applyBasicConfigurations(idleMode, inverted, isOpenRamp, rampRate, smartCurrentLimit);
        if (isAbsolute) {
            sparkFlexConfig.absoluteEncoder.positionConversionFactor(positionConversionFactor);
        } else {
            sparkFlexConfig.encoder.positionConversionFactor(positionConversionFactor);
        }
    }
    ...
    public void applyConfigurations(IdleMode idleMode, boolean inverted, boolean isOpenRamp, double rampRate, int smartCurrentLimit, boolean isAbsolute, double velocityConversionFactor, ControlType controlType) {
        applyBasicConfigurations(idleMode, inverted, isOpenRamp, rampRate, smartCurrentLimit);
        if (isAbsolute) {
            sparkFlexConfig.absoluteEncoder.velocityConversionFactor(velocityConversionFactor);
        } else {
            sparkFlexConfig.encoder.velocityConversionFactor(velocityConversionFactor);
        }
    ...
    public void applyConfigurations(IdleMode idleMode, boolean inverted, boolean isOpenRamp, double rampRate, int smartCurrentLimit, boolean isAbsolute, double positionConversionFactor, boolean positionWrappingEnabled,
        ...
        sparkFlexConfig.closedLoop.outputRange(-1, 1);
        sparkFlexConfig.closedLoop.positionWrappingEnabled(positionWrappingEnabled);
        sparkFlexConfig.closedLoop.positionWrappingInputRange(positionMinInput, positionMaxInput);
        sparkFlexConfig.closedLoop.pidf(P, I, D, FF)
    ...
    public void applyConfigurations(IdleMode idleMode, boolean inverted, boolean isOpenRamp, double rampRate, int smartCurrentLimit, boolean isAbsolute, double velocityConversionFactor, ControlType controlType,
        ...
        sparkFlexConfig.closedLoop.positionWrappingEnabled(positionWrappingEnabled);
        sparkFlexConfig.closedLoop.positionWrappingInputRange(positionMinInput, positionMaxInput);
        sparkFlexConfig.closedLoop.pidf(P, I, D, FF).maxMotion.maxAcceleration(maxAcceleration).maxVelocity(maxVelocity).allowedClosedLoopError(errorTolerance).positionMode(modeType);;
    }
    ...
    public SparkFlexConfig getApplied(){
        return this.sparkFlexConfig;
    }
}
